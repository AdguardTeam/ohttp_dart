...
- Responds to `gatewayUrl` POSTs using `decapExportedSecret` (HPKE KEM Decap) + `sealBhttpResponse` (HKDF + AES-128-GCM per RFC 9458 §4.6.2) from `test/stubs/gateway_stub.dart`. The stub also exposes `openEncapsulatedRequest`, which derives the HPKE request AEAD key and decrypts the inner BHTTP request ciphertext (used by the request-content round-trip test).
